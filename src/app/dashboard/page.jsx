...
export default function Dashboard() {
    ...
    if (authLoading || loading) {
        return (
            <div className="min-h-screen pt-24 px-6 animate-fade-in" style={{ backgroundColor: 'var(--color-bg)' }}>
    ...
    return (
        <div className="min-h-[calc(100vh-8rem)] pt-12 pb-24 px-6 font-mono animate-fade-in" style={{ backgroundColor: 'var(--color-bg)' }}>
            <div className="max-w-6xl mx-auto">
                {/* Header */}
                <div className="mb-12">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
                    {[
                        ...
                        <div key={i} className={`p-5 border ${cardBg}`} style={{ animationDelay: `${i*100}ms` }}>
                ...
                <div className="flex gap-6 mb-8 border-b" style={{ borderColor: 'var(--color-border)' }}>
                    {['designs', 'orders'].map(t => (
                        <button key={t} onClick={() => setTab(t)}
                                className={`pb-3 text-xs tracking-[0.3em] uppercase font-mono border-b-2 transition-colors cursor-pointer ${tab === t ? 'border-[#E85D26] text-[#E85D26]' : 'border-transparent'}`}
                    ...
                    <div>
                        {designs.length === 0 ? (
                            <div className="text-center py-20 animate-fade-in">
                                ...
                                <Link href="/configure/1" className="px-8 py-3 bg-[#E85D26] text-white text-xs tracking-[0.3em] uppercase hover:bg-[#D14F1E] transition-colors">
                                    Start Designing
                                </Link>
                            </div>
                        ) : (
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                                {designs.map((design, i) => (
                                    <div key={design.id} className={`border overflow-hidden group ${cardBg}`}
                                        ...
                                        <div className="p-5">
                                            ...
                                            <div className="flex gap-2">
                                                <Link href={`/configure/${design.shoe_id}`}
                                                ...
                                                <button onClick={() => deleteDesign(design.id)}
                                                        className="py-2 px-4 text-[10px] tracking-widest uppercase border border-red-300 text-red-500 hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors cursor-pointer">
                                                    ✕
                                                </button>
                                            </div>
                                            {design.share_token && (
                                                <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/shared/${design.share_token}`); addToast('Share link copied!', 'info'); }}
                                                        className="w-full mt-2 py-2 text-[10px] tracking-widest uppercase hover:text-[#E85D26] transition-colors cursor-pointer"
                    ...
                    <div>
                        {orders.length === 0 ? (
                            ...
                            <div className="space-y-4 animate-fade-in">
                                {orders.map((order, i) => (
                                    <div key={order.id} className={`p-6 border ${cardBg}`}
                                         style={{ animationDelay: `${i*80}ms` }}>
