...
export default function Auth() {
    ...
    return (
        <div className="min-h-[calc(100vh-8rem)] flex font-mono animate-fade-in" style={{ backgroundColor: 'var(--color-bg)' }}>
            {/* Left side — Brand visual */}
            <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden items-center justify-center bg-[#111111]">
                ...
                <div className="relative z-10 text-center px-12">
                    <div className="text-[10px] tracking-[0.5em] text-[#E85D26] mb-6 uppercase">Welcome to</div>
                    <h1 className="text-6xl xl:text-7xl font-bold text-white tracking-[0.15em] mb-6">
                        SOLE<span className="text-[#E85D26]">CRAFT</span>
                    </h1>
                    <p className="text-sm text-gray-400 tracking-[0.2em] uppercase max-w-sm mx-auto">
                        Design your perfect sneaker in real-time 3D
                    </p>
                    <div className="mt-12 flex justify-center gap-8 text-gray-500 text-[10px] tracking-[0.3em] uppercase">
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12">
                <div className="w-full max-w-md">
                    ...
                    {mode !== 'forgot' && (
                        <div className="flex mb-10 border-b" style={{ borderColor: 'var(--color-border)' }}>
                            <button onClick={() => { setMode('login'); setErrors({}); }}
                                    className={`flex-1 pb-4 text-xs tracking-[0.3em] uppercase font-mono transition-colors border-b-2 cursor-pointer ${mode === 'login' ? 'border-[#E85D26] text-[#E85D26]' : 'border-transparent'}`}
                                    style={mode !== 'login' ? { color: 'var(--color-text-muted)' } : {}}>
                                Sign In
                            </button>
                            <button onClick={() => { setMode('register'); setErrors({}); }}
                                    className={`flex-1 pb-4 text-xs tracking-[0.3em] uppercase font-mono transition-colors border-b-2 cursor-pointer ${mode === 'register' ? 'border-[#E85D26] text-[#E85D26]' : 'border-transparent'}`}
                    ...
                    {mode === 'forgot' && (
                        <div>
                            <button onClick={() => setMode('login')} className="text-xs tracking-widest uppercase mb-6 flex items-center gap-2 hover:text-[#E85D26] transition-colors cursor-pointer"
                            ...
                            {forgotSent ? (
                                ...
                                <form onSubmit={handleForgot}>
                                    ...
                                    <button type="submit" disabled={loading}
                                            className="w-full mt-6 py-4 bg-[#E85D26] text-white text-xs tracking-[0.3em] uppercase font-mono hover:bg-[#D14F1E] transition-colors disabled:opacity-50 cursor-pointer">
                        ...
                        <form onSubmit={handleLogin} className="space-y-5">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <button type="button" onClick={() => setShowPw(!showPw)}
                                            className="absolute right-3 top-1/2 -translate-y-1/2 text-xs uppercase tracking-wider cursor-pointer"
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button type="button" onClick={() => { setMode('forgot'); setErrors({}); }}
                                        className="text-[10px] tracking-widest uppercase text-[#E85D26] hover:underline cursor-pointer">
                                    Forgot password?
                                </button>
                            </div>
                            <button type="submit" disabled={loading} id="login-submit"
                                    className="w-full py-4 bg-[#E85D26] text-white text-xs tracking-[0.3em] uppercase font-mono hover:bg-[#D14F1E] transition-colors disabled:opacity-50 hover:shadow-[0_0_30px_rgba(232,93,38,0.3)] cursor-pointer">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button type="button" className={`py-3 border text-xs tracking-wider uppercase font-mono flex items-center justify-center gap-2 transition-colors cursor-pointer ${darkMode ? 'border-[#333] text-[#999] hover:border-[#555]' : 'border-[#E5E5E5] text-[#666] hover:border-[#ccc]'}`}>
                            ...
                            <p className="text-center text-[9px] tracking-wider uppercase text-gray-500">
