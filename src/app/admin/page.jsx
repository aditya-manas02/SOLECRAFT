...
export default function Admin() {
    ...
    return (
        <div className={`min-h-[calc(100vh-8rem)] pt-12 pb-24 font-mono animate-fade-in ${bgClass}`}>
                                ...
                                {orders.map(o => (
                                    <tr key={o.id} className="border-b" style={{ borderColor: 'var(--color-border)' }}>
                                        ...
                                        <td className="p-4 text-right space-x-2">
                                            {o.status === 'placed' && (
                                                <>
                                                    <button onClick={() => updateStatus(o.id, 'accepted')} className="px-3 py-1 bg-green-600 text-white hover:bg-green-700 cursor-pointer">Accept</button>
                                                    <button onClick={() => updateStatus(o.id, 'rejected')} className="px-3 py-1 bg-red-600 text-white hover:bg-red-700 cursor-pointer">Reject</button>
                                                </>
                                            )}
                                            {o.status === 'accepted' && (
                                                <button onClick={() => updateStatus(o.id, 'manufacturing')} className="px-3 py-1 bg-blue-600 text-white hover:bg-blue-700 cursor-pointer">Start Mfg</button>
                                            )}
                                            {o.status === 'manufacturing' && (
                                                <button onClick={() => updateStatus(o.id, 'shipped')} className="px-3 py-1 bg-purple-600 text-white hover:bg-purple-700 cursor-pointer">Ship</button>
                                            )}
                                            {o.status === 'shipped' && (
                                                <button onClick={() => updateStatus(o.id, 'delivered')} className="px-3 py-1 bg-[#E85D26] text-white hover:bg-[#D14F1E] cursor-pointer">Delivered</button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
